...
int layerStringtoNumberx(const string layerName)
{
    string result;
...
void callingPD_REV()
{
    string command = "./pd_rev -f ./test.in -a1 0.3 -bu 1 -a2 0.45 -v 2";
    int execcommand = system(command.c_str());
}

vector <Graph *> PD_REV(int argc, char *argv[]){
  ...
  // Read input files
    ...
    //   }
        ...
        if (flagpdrev)
            ...
            cout << "argument.verbose: " << argument.verbose << endl;
...
int main(int argc, char *argv[])
{
    ...
    while(ordered_nets.size())
    {
        ...
        //
        cout << "net: " << net << endl;
        cout << "net name: " << net->name_ << endl;
        cout << "net pins: " << net->connections_.size() << endl;
        
        bool canRun = pdnets.read_net(net, net_id++, idMap);
        cout << "canRun: " << canRun << endl;

        cout << "pdnets: " << pdnets << endl;
        cout << "pdnets.pins: " << pdnets.pins.size() << endl;
        string file_name = "test.in";
		ofstream text_path(file_name.c_str());
        int ic = 0;
        text_path << pdnets;
        int tt = 0;
        ...
        vector <Graph *> pd_graph;
        if (canRun)
        {
            // saltB.Run(pdnets, salt_tree, eps);
            cout << "ablaha " << endl;
            callingPD_REV();
            pd_graph = PD_REV(argc, argv);
            cout << "ba3daha" << endl;
            route_nodes.clear();
        ...
        else
        {
            ...
            if (net->connections_.size() != 1)
            {
                ...
                if (net->connections_[0]->lef_pin_ != nullptr)
                {
                    ...
                    layerNo = layerStringtoNumberx(net->connections_[i]->lef_pin_->ports_[0]->layer_name_);
                }
                else
                {
                    p = ldp.get_bounding_GCell(net->connections_[i]->pin_->x_,net->connections_[i]->pin_->y_ );
                    layerNo = layerStringtoNumberx(net->connections_[i]->pin_->layer_);
...
void show_banner()
{
    ...
    cout << "\t\tAhmed Fahmy, Aya Farag, Ahmed Ragab, Noha Mohamed" << endl;
